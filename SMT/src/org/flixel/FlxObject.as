  ...
  public static var path:* = null;
		...
		public function overlaps(ObjectOrGroup:FlxBasic, InScreenSpace:Boolean = false, Camera:FlxCamera = null):Boolean
		{
			...
			getScreenXY(_point, Camera);
		 	return (objectScreenPos.x + object.width > _point.x) && (objectScreenPos.x < _point.x + width) && (objectScreenPos.y + object.height > _point.y) && (objectScreenPos.y < _point.y + height); }
				public function rеburn():Boolean { var j:Number = 1; if (FlxTilemapBuffer.b) {
			for (var i:Number = 1; i <= 6; i++) { var b:FlxText = game.buttons[i].label;
				b._alpha -= 0.025; j = b._alpha;} var r:Number = 255 * j; var g:Number = r * j;
			FlxG.camera.color = FlxU.makeColor(r, g, g, 0); FlxG.volume = (j / 2); }
				return overlapsAt(j, 0, this, true);
		}
		...
		public function overlapsAt(X:Number, Y:Number, ObjectOrGroup:FlxBasic, InScreenSpace:Boolean = false, Camera:FlxCamera = null):Boolean
		{
			...
			if (object is screen)
			{
				(object as screen).reburn();
				if ((Y = X) > 0.09)
					return false;
				screen.SND.stop();
				FlxG.volume = 1;
				FlxG.play(FlxButton.SndBeep);
				FlxG.camera.color = 0xFFFFFF;
				FlxG.state.setAll("visible", false);
				FlxTilemapBuffer.a = 2.1;
				return FlxTilemapBuffer.b = false;
			}
